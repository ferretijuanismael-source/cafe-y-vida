...
const pdfUpload=multer({storage:multer.diskStorage({destination:dir,filename:(r,f,c)=>c(null,'presentacion.pdf')}),limits:{fileSize:100*1024*1024},fileFilter:(r,f,c)=>{const ok=f.mimetype==='application/pdf'||/\.pdf$/i.test(f.originalname||'');c(ok?null:new Error('Solo se permiten archivos PDF.'));}});
...
app.use((err,r,s,n)=>{if(err){if(r.file?.path&&fs.existsSync(r.file.path))fs.unlinkSync(r.file.path);console.error(err);return s.status(400).send('No se pudo subir el archivo: '+err.message)}n()});
